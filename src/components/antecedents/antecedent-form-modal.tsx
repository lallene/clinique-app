...
type Props = {
  open: boolean;
  antecedent?: AntecedentMedical | null;
  onClose: () => void;
...
export default function AntecedentFormModal({
  open,
  antecedent,
  onClose,
  onSaved,
}: Props) {
  const isEditMode = Boolean(antecedent?.idAntecedent);

  const [libelle, setLibelle] = useState(antecedent?.libelle || '');
  const [categorie, setCategorie] = useState(
    antecedent?.categorie || 'INFECTIEUX',
  );
  const [actif, setActif] = useState(antecedent?.actif ?? true);
        ...
        <div className="mb-8 flex items-start justify-between">
          <div>
            <h3 className="text-2xl font-black text-slate-900">
              {isEditMode ? 'Modifier un antécédent' : 'Nouvel antécédent'}
            </h3>
            <p className="mt-1 text-xs font-bold uppercase tracking-widest text-slate-400">
              Référentiel médical standardisé
            </p>
          </div>
